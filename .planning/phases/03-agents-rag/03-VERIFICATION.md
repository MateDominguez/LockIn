...
4. **LLM output quality** — ✅ **VERIFIED 2026-04-25.** Full 7-agent pipeline ran end-to-end with `GEMINI_FORCE_FLASH=true` on AAPL: Macro Oracle (regime=late_expansion, risk_on, 7/7 FRED indicators), Value Hunter (RIM valuation $104.47, ROIC 82%, 682-char thesis), Bear (independent bearish thesis 856 chars with distribution), Strategist (VeTO signals, fmp_transcript correctly in missing sources), Guardian (z_score=10.63, m_score=-0.94, no circuit breaker), Judge (p_success=0.90, recommendation=PASS, kelly=0.0), Optimizer (completed with PASS). Added `GEMINI_FORCE_FLASH` env var in `llm.py` to transparently fall back Pro→Flash when Pro quota=0 (free tier since 2026-04-01). All 147 unit tests pass.
